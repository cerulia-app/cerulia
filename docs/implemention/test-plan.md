...
- semantic validation matrix test
	- createCharacterSheet で resolved `sheetSchemaPin` の `baseRulesetNsid == rulesetNsid` を満たさない入力を reject すること
	- createCharacterSheet で `sheetSchemaPin.uri` が character-sheet-schema を指さない場合、または resolved record の CID が `sheetSchemaPin.cid` と一致しない場合に reject すること
	- createCharacterSheet で live read が `sheetSchemaPin.cid` と不一致でも verified pin cache に同 CID の schema があれば accept できること
	...
	- recordCharacterConversion で `targetSheetSchemaPin.uri` が character-sheet-schema を指さない場合、または resolved record の CID が `targetSheetSchemaPin.cid` と一致しない場合に reject すること
	- recordCharacterConversion で `targetSheetSchemaPin.baseRulesetNsid != targetRulesetNsid` を reject すること
	- recordCharacterConversion で `targetRulesetNsid == source.rulesetNsid` を reject すること
	- recordCharacterConversion で carry-forward した current state が target schema の required field を満たさない場合に reject すること。API は target-only field を自動生成しないこと
	...
	- createScenario / updateScenario で `recommendedSheetSchemaPin.uri` が character-sheet-schema を指さない場合、または resolved record の CID が `recommendedSheetSchemaPin.cid` と一致しない場合に reject すること
	- createScenario / updateScenario で `recommendedSheetSchemaPin.baseRulesetNsid != scenario.rulesetNsid` を reject すること
...
- scenario.recommendedSheetSchemaPin と rulesetNsid の整合 test
...
- conversion が新しい target sheet を作り、same branch の head を更新する test
	- accepted な conversion が `sourceSheetPin` と `targetSheetPin` を exact provenance として保存する test
...
- AppView OAuth route、signed internal auth、mirror session compensation の integration test
...
- public redaction matrix を record 行単位で検証する test
	...
	- live schema read が `sheetSchemaPin.cid` と不一致でも verified cache に同 CID の schema があれば `getCharacterBranchView` public mode が解決し、`structuredStats` を維持できること
	- live schema read も verified cache も `sheetSchemaPin` を解決できない場合、`getCharacterBranchView` public mode が解決し、`structuredStats` だけを省略すること
	- linked `scenario` が欠落しても `getCharacterBranchView` / `getSessionView` / `getHome` の session row が維持され、stored scenarioLabel が無い場合は label field だけを省略すること
	- `getPlayerProfileView` が unresolved public branch row を省略しても view 全体は失敗しないこと
	- `getCampaignView` が missing `sharedRuleProfileRefs` row と missing house block を省略しても view 全体は失敗しないこと
	- `scenario.list` / `scenario.getView` が unresolved `recommendedSheetSchemaPin` を browse-only として扱い、`hasRecommendedSheetSchema = false` を返すこと
	- owner-facing branch list が broken-head branch row を維持し、repair-needed state を出せること
...
- scenario に recommendedSheetSchemaPin が無い場合、scenario 起点の create CTA を出さないこと
