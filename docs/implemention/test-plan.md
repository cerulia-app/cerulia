...
- public redaction matrix を record 行単位で検証する test
	...
	- pinned `character-sheet-schema` が欠落しても `getCharacterBranchView` public mode が解決し、`structuredStats` だけを省略すること
	...
	- `scenario.list` / `scenario.getView` が unresolved `recommendedSheetSchemaRef` を browse-only として扱い、`hasRecommendedSheetSchema = false` を返すこと
