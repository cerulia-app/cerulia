...
| player profile | core | 卓前の自己紹介と公開 character 導線を渡す共有面 |
...
- recommendedSheetSchemaPin を持つ scenario だけが character-sheet-schema の direct chain で作成画面にナビゲートする
...
- character-conversion（same-owner の ruleset 跨ぎ変換 provenance）
- campaign（長期卓のオプション）
...
ここでは「他の人に見せる」体験を完成させる。public shared surface の root は character detail とする。standalone な public session 個別 route は置かない。player profile は character detail に次ぐ共有面として扱い、プレイヤー単位の public character collection をその中に含める。

player profile は `/players/[did]` の direct read で解決し、record は owner repo の `literal:self` singleton を使う。projection は public branch link や public list を補助できるが、direct read の正本を置き換えない。
...
- **整合性と通常利用の分離**: schema pin と rebase は canonical compatibility contract として残すが、AppView の通常利用では schema title と field 入力を前面に置き、rebase は recovery 扱いに留める
