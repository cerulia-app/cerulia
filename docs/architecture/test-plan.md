...
| B-3 | session invariants | PL が自分の session を書けること。他人の DID を含まず、他人 owner の branch を拒否すること |
...
| B-6 | conversion fence | source / target rulesetNsid と sheet version pin が記録され、cross-owner conversion を拒否すること |
...
| B-10 | rule-profile contract | rule-profile が public-only current-head として read/write され、retire/status axis を持たないこと |
...
| C-8 | rule overlay projection | public campaign / house projection が raw rule-profile を返さず、overlay summary だけを返すこと |
...
A-1 〜 A-6、B-1 〜 B-10、C-1 〜 C-8、D-1 〜 D-4 が green であること。
