...
`DATABASE_URL_DIRECT` は migration に限定し、通常の app traffic に使わない。`APP_ENV` は Cloud Run revision ごとに明示し、`api` では `AUTH_TRUSTED_PROXY_HMAC_SECRET` を Secret Manager から注入する。
