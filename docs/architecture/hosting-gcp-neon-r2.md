...
current repo が直接提供する job entrypoint は次だけである。
...
`DATABASE_URL_DIRECT` は migration に限定し、通常の app traffic に使わない。`APP_ENV` は Cloud Run revision ごとに明示する運用を原則にし、未指定時は Cloud Run metadata から production を推定してもよい。`api` では `AUTH_TRUSTED_PROXY_HMAC_SECRET` を Secret Manager から注入する。
...
## M1 backend rehearsal checklist
...
Neon branch restore 自体の automation はこの repo の責務に含めない。M1 では「restore 後に API を正常系へ戻せること」を runbook と smoke で固定する。
