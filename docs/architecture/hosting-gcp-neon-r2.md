...
| object storage | Cloudflare R2 | portrait、contract artifact など immutable object の保管 |
...
| AppView runtime | `APP_ENV`, `ORIGIN`, `INTERNAL_API_BASE_URL`, `INTERNAL_API_AUTH_HMAC_SECRET`, `SESSION_COOKIE_SECRET`, `SESSION_COOKIE_NAME`, `PUBLIC_APP_NAME` |
...
`DATABASE_URL_DIRECT` は migration と rebuild に限定し、通常の app traffic に使わない。`APP_ENV` は起動必須とし、Cloud Run revision ごとに明示する。未指定または未知値のときは起動を停止する。`api` では `AUTH_TRUSTED_PROXY_HMAC_SECRET` を Secret Manager から注入する。AppView では `ORIGIN` を canonical public origin に固定し、staging / prod の signed-in owner request では `INTERNAL_API_AUTH_HMAC_SECRET` を必須にする。

M1 backend の `api` が直接読む値は `APP_ENV`, `HTTP_ADDR`, `PUBLIC_BASE_URL`, `AUTH_TRUSTED_PROXY_HMAC_SECRET`, `AUTH_TRUSTED_PROXY_MAX_SKEW`, `DATABASE_*`, `LOG_LEVEL`, `SHUTDOWN_TIMEOUT`, `MIGRATIONS_DIR`, `BLOB_*`, `R2_*` に限る。`ORIGIN`, `INTERNAL_API_BASE_URL`, `INTERNAL_API_AUTH_HMAC_SECRET`, `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, `SESSION_COOKIE_SECRET`, `OTEL_EXPORTER_OTLP_ENDPOINT` は AppView または将来の周辺 runtime 向け値であり、現 M1 backend では未使用である。
...
- `./scripts/smoke.ps1 -RulesetNsid <known-ruleset> -SheetSchemaRef <known-schema-ref>` を実行し、anonymous public read と authenticated core mutation を通す。restore rehearsal では `-ReadOnly` を使わない
