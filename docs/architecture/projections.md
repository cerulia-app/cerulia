...
`getCampaignView` は owner / maintainer mode では authenticated reader、public / anonymous mode では auth bundle なしの public lens で解決する。
...
`getHouseView` は owner / maintainer mode では authenticated reader、public / anonymous mode では auth bundle なしの public lens で解決する。
