...
- owner mode: 認可済みの continuity owner が見る
...
character home は、owner が自分の continuity 資産を最初に確認するための既定 home である。
...
- 各 branch に紐づく recent session summary
- 各 branch または session に紐づく publication current heads
...
campaign view は、campaign を scope として読むための shared view である。shared rule chain、published artifact を確認する面として扱う。public mode では、active な public publication current head を索引する shell / index として振る舞う。
...
- campaign に属する recent session summary
...
| recent session summary | required | excluded |
| active branch summary | optional | excluded |
| maintainer summary | optional | excluded |
| archived counts | optional | excluded |
...
- owner mode では campaign 配下の非 public session / branch summary を返してよい
...
- subject が converted branch の場合は、その current character-conversion の最小 summary
...
- owner mode でも既定は active current head だけを返し、retired current head は explicit な includeRetired opt-in で archived summary として返してよい
