...
プロフィール内のシナリオタブとシナリオ参照ペインに、owner が登録したシナリオを表示する。
...
recommendedSheetSchemaPin の解決に失敗した scenario も readable な参照用 entry として残す。create CTA は出さず、schema 解決に依存する導線だけを止める。
