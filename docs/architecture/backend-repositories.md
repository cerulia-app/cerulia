...
- permission bundle の解決、owner / maintainer / public mode の判定、visibility の authoritative judgment は `api` に置く
...
- Cerulia product-core は disclosure / secret payload を現時点で扱わない。将来導入しても `protocol` や `projection` に平文 ownership を置かない
