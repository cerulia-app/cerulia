...
AppView は、schema-backed な character authoring、post-run session record、canonical な shared character detail を、PL-first の境界でまとめる locale-aware な surface である。

## AppView が成立したと言える状態

MVP で最初に成立したと言えるのは、PL がキャラクターを作り、卓のあとに session を継続して記録し、次の卓で同じ character detail を再共有できる状態である。
...
| Who | 最優先は、自分のキャラクターを作り育てる PL。次点で、共有リンクを確認する GM / 卓相手。さらに次点で、scenario や system 情報を登録する maintainer。細かなアクセス制御、承認、監査を強く求める運用者は最適化の主対象にしない |
| What | public top、character create、character detail、session record、sessions workbench を中心に、PL の owner flow と shared read flow を提供する。scenario / campaign / house は scope 内だが主役ではない。public profile は secondary later とし、canonical shared surface は character detail のまま固定する |
...
| Where | public surface は `/` と character detail を中心にログイン不要で開ける。owner flow は sign-in 後の workbench で扱う。将来的に public profile を追加しても、shared entrypoint は character detail に残す |
...
- public profile を canonical shared surface にすること

## Secondary Later

次は scope 内だが、MVP の canonical flow を固定した後に扱う。

- player 単位の public profile
- public character collection
- public profile から各 character detail へ束ねる導線
...
- public profile を将来追加しても、「誰のアカウントか」より「どのキャラクターか」を先に伝える
