...
generic な session deep-link の precedence は `participant-shell -> governance-console -> appeal-only -> join -> public-replay -> sign-in -> no-access` に固定する。`retired-carrier` は carrier-specific deep-link preflight を追加した段階で使う予約値とし、現在の `sessionRef` ベース preflight では到達させない。
