...
- `/home` が signed-in の継続作業 workbench
...
- retired branch の `/characters/:branchRef` は read-only historical detail とし、edit CTA を出さない。draft なら一覧には出さず、direct link で draft 状態付き表示にする
