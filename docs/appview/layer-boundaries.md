...
1. **controlled repo write only**: AppView mutation は caller が control する repo にだけ書く。character / session 系は caller の personal repo、campaign / house / scenario / rule-profile / schema 系は caller が owner または maintainer として管理する shared-maintained repo に限る
