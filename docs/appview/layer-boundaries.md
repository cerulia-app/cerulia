...
10. **character detail remains canonical**: player profile を提供しても、canonical shared surface は character detail のままにする。player profile は自己紹介と各 character detail への導線を束ねる
...
12. **profile composition boundary**: player profile の Bluesky 既存項目は fallback 読み取り、Cerulia 側の明示上書きがある場合のみ差し替える。TRPG 固有項目は任意入力で開始可能に保つ
13. **social delegation**: follow graph、timeline、通知は Cerulia の責務に入れず、Bluesky 側に委譲する。Cerulia は任意投稿連携のみ扱う
