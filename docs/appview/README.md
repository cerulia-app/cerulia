...
| profile workbench | `/profile/[actor]` のタブで自分の character / session / scenario / campaign / house を確認し、キャラクター以外はペイン / シートで詳細と編集を行う |
| scenario registry | owner が登録したシナリオの一覧・登録。検索 surface は持たない |
| public profile | `/profile/[actor]` の自己紹介面。`actor` は DID または handle。Bluesky fallback と Cerulia 任意入力を合成し、公開 character collection への導線を束ねる |
| campaign view | プロフィール内のキャンペーンペイン。長期卓のセッション一覧と rule overlay を表示する |
| house detail | プロフィール内のハウスペイン。house metadata、ハウスルール、activity summary を表示する |
