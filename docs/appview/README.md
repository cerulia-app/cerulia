...
| character detail | キャラクターの詳細、stats、立ち絵、公開セッション履歴、成長履歴。conversion provenance は post-MVP の optional 表示として扱う。schema-less の場合は公開 structured stats を省略する |
...
| public profile | player 単位の自己紹介面。Bluesky fallback と Cerulia 任意入力を合成し、公開 character への導線を束ねる |
