...
- public profile は secondary later に留め、target MVP では shared root にしない
...
### 1. MVP の public root は character detail である

Cerulia の canonical shared surface は character detail であり、MVP ではプレイヤー単位の public profile を置かない。
将来的に public profile を追加しても、画面が「誰のアカウントか」よりも「どのキャラクターか」を主語にしていることを優先する。
...
| 案 | 強み | 主な問題 | 判定 |
| --- | --- | --- | --- |
| SNS プロフィール直系 | 見慣れていて把握が速い | feed 化しやすく、player profile を作りたくなる | 不採用 |
...
8. public profile を後で追加しても、shared root を character detail から動かさない
