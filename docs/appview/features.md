...
| ------------------------------ | ------------------------------------------------------------- | ------------------------------------------ | -------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------- |
| character studio               | new sheet、import、branch、convert を迷わず始められる         | branch owner                               | signed-in home、characters、character detail | getCharacterHome | importCharacterSheet、createCharacterBranch、recordCharacterConversion（convert accept は backend surface が揃うまで fail-closed） |
| character history workbench    | いまの版、recent session、publication をひと目で読める    | branch owner                               | signed-in home、character detail             | getCharacterHome | recordCharacterAdvancement、publishSubject、retirePublication |
| campaign workspace             | shared rule chain とセッション履歴を理解しやすい                    | campaign owner / maintainer                | campaigns、campaign detail                   | getCampaignView  | createCampaign、attachRuleProfile、retireRuleProfile                                  |
| publication library            | 公開中の版を読み、何を見せているかを管理できる                | owner、public reader                        | publications、publication detail             | listPublications | publishSubject、retirePublication                                                     |
...
create flow の最後では、次の 2 点を review する。
...
| 機能                    | 置く場所                              | 理由                                                          |
| ----------------------- | ------------------------------------- | ------------------------------------------------------------- |
| reader lens cue         | primary nav と surface intro          | public と owner の lens を nav と copy で混ぜないため |
...
- recent session、recent conversion、publication row を並列に読める
...
- owner mode では rule provenance を読める
