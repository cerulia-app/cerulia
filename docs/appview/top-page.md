...
Cerulia のトップページは 2 つある。anonymous が開く `/` と、sign-in 後の既定入口である `/home` である。両者は別の役割を持つが、どちらも character continuity service という同じ product thesis を共有する。

## `/` の役割

`/` は public entry shell であり、次の 3 点だけを最初の 1 画面で伝える。

- Cerulia は「作る、続ける、持ち運ぶ、見せる」をつなぐ continuity service であること
...
`/home` は signed-in user の continuity workbench である。既定では次の 4 block を持つ。
...
| signed-in owner-steward | public lens のまま閲覧できる | continuity workbench を表示する |
