...
CTA のURL固定値:
- primary: locale-aware な `/sign-in` route
- secondary: public の `characters/[branch]` 実例
- tertiary: public の `profile/[actor]` 実例

実例 route がまだ存在しない段階では、placeholder の public URL を置かない。
この段階の secondary / tertiary CTA は `#split-character` と `#split-profile` の in-page anchor に退避し、対応 route の実装後に public 実例へ差し替える。
