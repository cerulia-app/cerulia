...
describe("normalizePathname", () => {
	...
	it("strips a trailing slash from non-root paths", () => {
		...
		expect(normalizePathname("characters/main-branch/")).toBe(
			"/characters/main-branch",
		);
	});
});
...
describe("localizePathname", () => {
	it("keeps Japanese routes unprefixed", () => {
		...
		expect(localizePathname("/profile/example.com", "ja")).toBe(
			"/profile/example.com",
		);
	});
	...
	it("prefixes non-default locales", () => {
		...
		expect(localizePathname("/characters/new", "zh")).toBe(
			"/zh/characters/new",
		);
	});
});
