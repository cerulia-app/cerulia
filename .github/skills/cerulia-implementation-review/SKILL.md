...
## Review Mindset
- reviewer agent に丸投げするのではなく、先に自分で対象を読み、一次レビューを行う
- 一次レビューでは、怪しい境界、壊れそうな挙動、契約ズレ、未検証の前提から candidate findings と仮説を作る
- reviewer agents は、その仮説を確認、反証、補強し、自分が見落とした論点を拾うために使う
- reviewer output はそのまま採用せず、必要に応じて元のコード、docs、tests、config に戻って再確認する
...
2. 対象コード、近接する tests、docs、config、entrypoints を自分で先に読み、主要な risk area と candidate findings を仮説として整理する。仮説ごとに、疑っている root cause、影響範囲、未確認点を短くメモする。
...
5. 自分の仮説と reviewer から返ってきた findings を root cause 単位で統合する。同じ問題を複数観点が指摘している場合は重複を消し、重なり自体に意味があるときだけ Cross-Cutting Notes に残す。自分の仮説が reviewer や元コードと矛盾した場合は、そのまま残さず再確認して修正する。
6. 重大度と実装リスクで並べ替え、開発者が今対応すべき短いリストに圧縮する。意味のある問題が残らなければ、そのことを明示する。
7. どの reviewer agents を走らせ、どこを見たかに加え、どの仮説を検証したかを Coverage に書く。必要な reviewer agent を実行できなかった場合も黙って省略せず Coverage で明示する。

## Quality Bar
- reviewer agent を呼ぶ前に、自分で一次レビューして仮説を作る
- reviewer output を盲信せず、元のコード、docs、tests、config に戻って検証する
