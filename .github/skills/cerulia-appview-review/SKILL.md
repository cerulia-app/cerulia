...
- 一次レビューでは、境界逸脱、local truth reconstruction、debuggability の悪化、UI の不明瞭化、テストの偽りの安心感について candidate findings と仮説を作る
...
3. 次の branching で reviewer agents を選ぶ。scope だけでなく、自分の仮説と未確認点に照らして必要な reviewer を決める。
   - appview 全体、広い PR、複数 route、shared shell、または境界と UI の両方にまたがる変更なら 6 つすべてを呼び出す。
...
- frontend-local truth reconstruction、meaningless fallback、swallowed error、public/private lens leakage、archive/current grammar 混同、generic dashboard や session-centric UI の残骸、半端な stub shell を軽い指摘に落とさない
...
- /cerulia-appview-review この AppView PR 全体を 6 観点でレビューして、重複をまとめて優先度順に返す
